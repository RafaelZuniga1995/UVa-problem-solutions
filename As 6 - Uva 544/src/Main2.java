		...
		while (true) {
			...
			for (int i = 0; i < r; i++) {
				...
				if (!enumerate.containsKey(city1)) {
					// sSystem.out.println(V + " " + city1);
					enumerate.put(city1, V++);

				}
				if (!enumerate.containsKey(city2)) {
					// System.out.println(V + " " + city2);
			...
			// System.out.println("city keyset: " + cities.keySet().size());
			for (String s : cities.keySet()) {
				...
				for (int i = 0; i < edgeTo.size(); i += 2) {
					...
					// System.out.println( "(" + enumerate.get(s) + ", " +
					// enumerate.get(edgeTo.get(i)) + ", " + weight + ")");
