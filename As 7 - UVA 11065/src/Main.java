...
public class Main {
	...
	// to identify if a permutation is an independent set
	...
	/*
	 * Finds every subset in the power set of the vertices set in graph G and
	 * determines whether the subset is an independent set or not.
	 */
	private void permute(int n) {
		...
		if (n <= 0) {
			...
			// vertices in independent set, or vertices that are adjacent to a
			// vertex
			...
			for (int i = 0; i < bits.length; i++) {
				...
				if (bits[i] == 1) {
					...
					for (Edge e : G.adj(i)) {
						...
						// if node w in edge (i-w) is already in the hash set
			...
			// update total maximal independent sets founds so far and maximum
			// set.
			if (marked.size() == V) {
	 ...
	 * type {@link Edge} and has a real-valued weight. It supports the following
	 * two primary operations: add an edge to the graph, iterate over all of the
	 * edges incident to a vertex. It also provides methods for returning the
	 * number of vertices <em>V</em> and the number of edges <em>E</em>.
	 ...
	 * time (in the worst case) except iterating over the edges incident to a
	...
	public class EdgeWeightedGraph {
		...
		/**
		 * Returns all edges in this edge-weighted graph. To iterate over the
		 * edges in this edge-weighted graph, use foreach notation:
	...
	/**
	 * The <tt>Bag</tt> class represents a bag (or multiset) of generic items.
	...
	public class Bag<Item> implements Iterable<Item> {
		 ...
		 * arbitrary order.
		 *
		 * @return an iterator that iterates over the items in this bag in
		 *         arbitrary order
		 */
	 ...
	 * {@link EdgeWeightedGraph}. Each edge consists of two integers (naming the
	 * two vertices) and a real-value weight. The data type provides methods for
	 * accessing the two endpoints of the edge and the weight. The natural order
